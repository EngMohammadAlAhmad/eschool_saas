...
class ChildFeeDetails {
  ...
  double getOutstandingInstallmentAmount() {
    if (hasAnyDueInstallment()) {
      ...
      for (var installment in dueInstallments()) {
        outstandingAmount +=
            (installment.dueChargeAmount ?? 0.0) + installment.remainingAmount;
      }
  ...
  double remainingInstallmentAmount() {
    ...
    for (var installment in (installments ?? [])) {
      totalPaidAmount += (installment.paidAmount ?? 0.0);
    }

    final remaining = totalAmount - totalPaidAmount;
  ...
  double maximumAdvanceInstallmentAmount() {
    // Calculate total amount already paid across all installments
    double totalPaidAmount = 0.0;
    for (var installment in (installments ?? [])) {
      totalPaidAmount += (installment.paidAmount ?? 0.0);
    }
    ...
    // Calculate the remaining amount for the next installment
    ...
    // Calculate outstanding base amounts from due installments (remaining only).
    // Due charges are penalties on top of totalCompulsoryFees, so they must NOT
    // be subtracted from the base fee total when computing the advance amount.
    double outstandingBaseAmount = 0.0;
    for (var installment in dueInsts) {
      outstandingBaseAmount += installment.remainingAmount;
    }
